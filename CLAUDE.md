...
- Enforcement hooks (`plugin/hooks/hooks.json`): dangerous-command gate (PreToolUse), verify-before-stop + lesson-capture backstop (Stop), telemetry (SubagentStop), lesson-capture backstop again on `PreCompact` (BL-0104 — a session that compacts mid-conversation no longer skips the DR-047 rule-8 re-scan).
