...
**Iterate without advancing (DR-032).** No manual phase (`explore`, `new-idea`, `spec`, `design`, `blueprint`) auto-advances a column: it produces its output, sets `advance_pending: true` and waits for your "ok, advance." **Re-running the same phase = keep polishing** (it refines, it doesn't regenerate or repeat what was discarded). The essence of the back-and-forth is persisted in the project's `docs/iteration.md` (for ideas without a project: `factory/ideas/_drafts/<slug>.md`), so you can pick it up again even if you lose the conversation. It does not apply to `implement` (autonomous, already resumable) and does not change the production gate.
