#!/usr/bin/env bash
set -euo pipefail
PROJ="${1:-.}"; MAX_SPEND="${2:-12}"; MAX_DURATION="${3:-7200}"; MAX_RETRIES="${4:-2}"; MAX_BLOCKS="${5:-3}"; CHANGE="${6:-}"; FRDS="${7:-}"; RUN_MODE="${8:-auto}"; LAUNCH_MODE="${9:-${PANDACORP_CODEX_LAUNCH_MODE:-background}}"; CERT_AUTH="${10:-}"; ROOT=$(cd "$(dirname "$0")/.." && pwd); PROJECT=$(cd "$PROJ" && pwd -P)
CERT_CONSUMED=0
ATTENDED_PERMIT=""
ATTENDED_SECRET=""
revoke_permits() { if [ "$CERT_CONSUMED" = "1" ]; then if node "$ROOT/runtime/codex/certification-permit.mjs" --mode terminal "${CERT_ARGS[@]}" --run-id "$RUN_ID" --terminal-reason "launcher-exit-${result:-3}" >/dev/null; then CERT_CONSUMED=0; else return 1; fi; fi; if [ -n "$ATTENDED_PERMIT" ]; then node "$ROOT/runtime/codex/attended-permit.mjs" --mode terminal --project "$PROJECT" --permit "$ATTENDED_PERMIT" --reason "launcher-exit-${result:-3}" >/dev/null || return 1; fi; }
trap revoke_permits EXIT
...
EXECUTION_PROFILE=""
if [ -z "$CERT_AUTH" ]; then
  [ "$IMPLEMENT_STATUS" = "EXPERIMENTAL" ] || { echo "ERROR: Codex implement is $IMPLEMENT_STATUS; normal execution is unavailable" >&2; exit 3; }
  [ "$LAUNCH_MODE" = "foreground" ] || { echo "ERROR: EXPERIMENTAL Codex implement is attended foreground only" >&2; exit 3; }
  if { [ -n "$CHANGE" ] && [ -n "$FRDS" ]; } || { [ -z "$CHANGE" ] && [ -z "$FRDS" ]; }; then echo "ERROR: attended_foreground requires exactly one change or FRD target" >&2; exit 3; fi
  [ "$MAX_DURATION" -le 7200 ] || { echo "ERROR: attended_foreground max-duration must be <= 7200 seconds" >&2; exit 3; }
  EXECUTION_PROFILE="attended_foreground"
else
  EXECUTION_PROFILE="certification"
fi
if [ -n "$CERT_AUTH" ]; then
...
if [ "$EXECUTION_PROFILE" = "certification" ]; then node "$ROOT/scripts/preflight-codex-unattended.mjs" "$PROJECT"; fi
...
if [ "$EXECUTION_PROFILE" = "attended_foreground" ]; then
  ATTENDED_SECRET=$(node -e 'process.stdout.write(require("node:crypto").randomBytes(32).toString("hex"))')
  ATTENDED_JSON=$(printf '%s' "$ATTENDED_SECRET" | node "$ROOT/runtime/codex/attended-permit.mjs" --mode issue --secret-fd 0 --launcher-pid "$$" --project "$PROJECT" --run-id "$RUN_ID" --change "$CHANGE" --frds "$FRDS" --max-spend "$MAX_SPEND" --max-duration "$MAX_DURATION" --max-retries "$MAX_RETRIES" --max-blocks "$MAX_BLOCKS")
  ATTENDED_PERMIT=$(node -e 'process.stdout.write(JSON.parse(process.argv[1]).path)' "$ATTENDED_JSON")
fi
LOG="$PROJECT/.pandacorp/run/codex-executor.log"; mkdir -p "$PROJECT/.pandacorp/run"
CMD=(node "$ROOT/runtime/codex/supervisor.mjs" --project "$PROJECT" --run-id "$RUN_ID" --max-spend "$MAX_SPEND" --max-duration "$MAX_DURATION" --max-retries "$MAX_RETRIES" --max-blocks "$MAX_BLOCKS" --execution-profile "$EXECUTION_PROFILE")
[ -n "$CHANGE" ] && CMD+=(--change "$CHANGE")
[ -n "$FRDS" ] && CMD+=(--frds "$FRDS")
if [ -n "$CERT_AUTH" ]; then CMD+=(--certification-receipt "$CERT_RECEIPT"); fi
if [ "$EXECUTION_PROFILE" = "attended_foreground" ]; then CMD+=(--attended-permit "$ATTENDED_PERMIT" --attended-fd 9); fi
if [ "$EXECUTION_PROFILE" = "attended_foreground" ]; then "${CMD[@]}" 9<<<"$ATTENDED_SECRET" >>"$LOG" 2>&1 & ATTENDED_SECRET=""; elif [ "$LAUNCH_MODE" = "background" ]; then nohup "${CMD[@]}" >>"$LOG" 2>&1 & else "${CMD[@]}" >>"$LOG" 2>&1 & fi
SUPERVISOR_PID=$!
INHIBITOR_PID=0
if [ "$EXECUTION_PROFILE" = "attended_foreground" ]; then
  : # The attached launcher is the liveness owner; no unattended sleep-inhibitor claim.
...
sleep 1
if ! kill -0 "$SUPERVISOR_PID" 2>/dev/null || { [ "$INHIBITOR_PID" -gt 0 ] && ! kill -0 "$INHIBITOR_PID" 2>/dev/null; }; then
  kill -TERM "$SUPERVISOR_PID" 2>/dev/null || true; [ "$INHIBITOR_PID" -gt 0 ] && kill -TERM "$INHIBITOR_PID" 2>/dev/null || true; wait "$SUPERVISOR_PID" 2>/dev/null || true; [ "$INHIBITOR_PID" -gt 0 ] && wait "$INHIBITOR_PID" 2>/dev/null || true
...
if [ "$LAUNCH_MODE" = "foreground" ]; then
  ...
  stop_children() {
    ...
    kill -TERM "$SUPERVISOR_PID" 2>/dev/null || true; [ "$INHIBITOR_PID" -gt 0 ] && kill -TERM "$INHIBITOR_PID" 2>/dev/null || true
    for _ in $(seq 1 50); do kill -0 "$SUPERVISOR_PID" 2>/dev/null || break; sleep 0.1; done
    kill -KILL "$SUPERVISOR_PID" 2>/dev/null || true; [ "$INHIBITOR_PID" -gt 0 ] && kill -KILL "$INHIBITOR_PID" 2>/dev/null || true
  }
...
node -e 'const fs=require("node:fs");const [file,runId,supervisorPid,inhibitorPid,launcherPid,launchMode,profile,started,...rest]=process.argv.slice(1);const split=rest.indexOf("--supervisor-argv");if(split<0)process.exit(3);const resumeArgv=rest.slice(0,split),supervisorArgv=rest.slice(split+1);const value={run_id:runId,pid:Number(supervisorPid),supervisor_pid:Number(supervisorPid),sleep_inhibitor_pid:Number(inhibitorPid),launcher_pid:Number(launcherPid),launch_mode:launchMode,execution_profile:profile,runtime:"codex",started_at:started,resume_argv:resumeArgv,supervisor_argv:supervisorArgv};const tmp=`${file}.tmp-${process.pid}`;fs.writeFileSync(tmp,`${JSON.stringify(value,null,2)}\n`,{mode:0o600});fs.renameSync(tmp,file);' "$RECEIPT" "$RUN_ID" "$SUPERVISOR_PID" "$INHIBITOR_PID" "$$" "$LAUNCH_MODE" "$EXECUTION_PROFILE" "$(date -u +%FT%TZ)" "${RESUME_ARGV[@]}" --supervisor-argv "${CMD[@]}"
...
if [ "$LAUNCH_MODE" = "foreground" ]; then
  ...
  if [ "$INHIBITOR_PID" -gt 0 ]; then kill -TERM "$INHIBITOR_PID" 2>/dev/null || true; wait "$INHIBITOR_PID" 2>/dev/null || true; fi
  if [ -n "$CERT_AUTH" ]; then
