#!/usr/bin/env bash
set -euo pipefail
PROJ="${1:-.}"; MAX_SPEND="${2:-12}"; MAX_DURATION="${3:-21600}"; MAX_RETRIES="${4:-2}"; MAX_BLOCKS="${5:-3}"; CHANGE="${6:-}"; FRDS="${7:-}"; RUN_MODE="${8:-auto}"; LAUNCH_MODE="${9:-${PANDACORP_CODEX_LAUNCH_MODE:-background}}"; ROOT=$(cd "$(dirname "$0")/.." && pwd); PROJECT=$(cd "$PROJ" && pwd -P)
...
node -e 'const fs=require("node:fs");const [file,runId,supervisorPid,inhibitorPid,launcherPid,launchMode,started,...rest]=process.argv.slice(1);const resumeArgv=rest.slice(0,11),supervisorArgv=rest.slice(11);const value={run_id:runId,pid:Number(supervisorPid),supervisor_pid:Number(supervisorPid),sleep_inhibitor_pid:Number(inhibitorPid),launcher_pid:Number(launcherPid),launch_mode:launchMode,runtime:"codex",started_at:started,resume_argv:resumeArgv,supervisor_argv:supervisorArgv};const tmp=`${file}.tmp-${process.pid}`;fs.writeFileSync(tmp,`${JSON.stringify(value,null,2)}\n`,{mode:0o600});fs.renameSync(tmp,file);' "$RECEIPT" "$RUN_ID" "$SUPERVISOR_PID" "$INHIBITOR_PID" "$$" "$LAUNCH_MODE" "$(date -u +%FT%TZ)" "${RESUME_ARGV[@]}" "${CMD[@]}"
