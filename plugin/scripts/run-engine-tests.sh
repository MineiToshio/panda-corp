...
EXPLICIT_SH_SUITES=(test-verify-gate-report.sh test-verify-before-stop.sh test-classify-change.sh test-check-derived-drift.sh test-doc-lint-closing.sh test-change-now-prose.sh test-worktree-bootstrap.sh test-preflight-version-skew.sh test-warn-adhoc-write.sh)
