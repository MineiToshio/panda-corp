...
await test("exact post-dispatch exec rollout classifies a silent exit as usage_limit with reset_at", async () => {
  const fx = await fixture(); const codexHome = path.join(fx.project, "codex-home"); const result = await execute(fx, "rollout-limit", [], { CODEX_HOME: codexHome }); ok(result.code === 25, `exit ${result.code}`); const cp = await checkpoint(fx.project); ok(cp.uncertain.error_class === "usage_limit" && isNearFutureReset(cp.uncertain.reset_at), JSON.stringify(cp)); const events = (await read(path.join(fx.project, ".pandacorp/run/test-events.ndjson"))).split("\n").filter(Boolean).map(JSON.parse); const uncertain = events.find((entry) => entry.semantic_name === "dispatch.uncertain"); ok(isNearFutureReset(uncertain?.data?.reset_at), JSON.stringify(uncertain));
});

await test("explicit reached-limit telemetry classifies usage_limit even below 100 percent", async () => {
  const fx = await fixture(); const result = await execute(fx, "rollout-reached", [], { CODEX_HOME: path.join(fx.project, "codex-home") }); ok(result.code === 25, `exit ${result.code}`); const cp = await checkpoint(fx.project); ok(cp.uncertain.error_class === "usage_limit" && isNearFutureReset(cp.uncertain.reset_at), JSON.stringify(cp));
});
