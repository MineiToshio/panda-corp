...
await test("answered decisions reopen only targeted FRDs and pending count is exact",async()=>{const fx=await fixture();const a=path.join(fx.project,"docs/frds/frd-01-a/work-orders/wo-01.md");await writeFile(a,(await read(a)).replace("implementation_status: PLANNED","implementation_status: BLOCKED\nblocked_reason: needs-owner"));const bRoot=path.join(fx.project,"docs/frds/frd-02-b");await mkdir(path.join(bRoot,"work-orders"),{recursive:true});await writeFile(path.join(bRoot,"frd.md"),"---\nimplementation_status: PLANNED\n---\n");await writeFile(path.join(bRoot,"blueprint.md"),"---\nimplementation_status: PLANNED\n---\n\n## Build Plan\n\n| WO | Depends on | Artifacts | Foundation | Parallel with |\n|---|---|---|---|---|\n| WO-02 | — | `b.js` | false | — |\n");const b=path.join(bRoot,"work-orders/wo-02.md");await writeFile(b,"---\nid: WO-02\nimplementation_status: BLOCKED\nblocked_reason: needs-owner\ndependsOn: []\n---\n");await writeFile(path.join(fx.project,".pandacorp/inbox/decisions.md"),"## A (resuelto)\n- **Bloquea:** WO-01\n- **Estado:** RESUELTO: sí\n\n## B (resuelto)\n- **Bloquea:** WO-02\n- **Estado:** RESUELTO: sí\n\n## C pendiente\n- **Bloquea:** WO-99\n- **Estado:** PENDIENTE\n");await run("git",["add","-A"],fx.project);await run("git",["commit","-qm","test: seed scoped decisions"],fx.project);const beforeB=await read(b);const result=await execute(fx,"success",["--frds","frd-01-a","--execution-profile","attended_foreground"]);ok(result.code===0,result.err);ok((await read(b))===beforeB,"out-of-scope WO changed");const status=await read(path.join(fx.project,".pandacorp/status.yaml"));ok(/^pending_decisions:\s*1$/m.test(status),status);});
