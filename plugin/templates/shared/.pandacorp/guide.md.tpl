...
|---|---|
| "add this feature" / "change this behavior" / "I found this bug" | **`/pandacorp:change`** — the single front door (DR-069): it classifies feature-vs-bug, assigns the class of service, derives the change's **rigor** with a deterministic script and files it in the queue `.pandacorp/inbox/changes/`. It does not only capture: for a **micro/normal** change with **no build running** it then implements and closes it in the same turn (`--now`, the default: a delegated subagent in its own worktree, the level's `verify.sh` gate, an independent reviewer on L1, and a batch close-out via `/pandacorp:sync --close-out`). It falls back to **capture-only** whenever the change is `critical` or touches the floor (auth, money, PII, persistence, irreversible, secrets/CI, the oracles, the factory's machinery), whenever a build is active (the engine is the sole writer while it runs), or on `--queue`; then the build drains it at its next safe point. `iterate`/`bug` are its internal engines — never route the owner to them directly (a direct `iterate` edits FRDs/WOs immediately, the exact concurrent-doc-edit hazard the queue removes) |
...
8. **Capture lessons as you work (self-learning) — IN THE SAME TURN, not at the end.** The moment a capture event happens — the owner corrects you, a fix lands after a failure, a library proves itself or fails, you hit a non-obvious gotcha — jot a one-line candidate to `.pandacorp/run/lessons.md` **in that same turn** (gitignored scratch; tag `(owner-stated)` if the owner said it, else `(agent-inferred)`). Don't polish it inline; never defer capture to the session's end (sessions span hours — deferred capture is lost capture; the Stop-hook backstop only re-scans what slipped). The factory's `librarian` later refines these into reusable lessons that make future projects faster. Capture freely; nothing is promoted without the owner.
   **And retrieve before you build (loop v2):** before non-trivial work, read **`{{FACTORY_PATH}}/factory/memory/INDEX.md`** — one line per active lesson with its "use when" trigger — and open the full `LESSON-NNNN` files whose trigger matches your task. When a lesson materially informs your work, **cite its `LESSON-NNNN` in the durable artifact you write** (blueprint, ADR, review, WO Status Note, progress log); the build close-out counts those citations deterministically (`count-lesson-citations.sh`) — never edit a lesson's `times_applied`/`applied_in` by hand.
...
10. **Interaction style with the owner (DR-110).** Scope: the lead agent's owner-facing replies only (what reaches the owner's chat) — NOT product/UI copy (stays with the `copywriter` agent + i18n) and NOT orchestrator↔subagent communication (`Agent`/`Task` prompts and results stay whatever shape serves the agents involved). When replying to the owner: mark where the real answer begins after progress narration (`---` + a short bold label); default concise, scannable and high-level (headers/bold/bullets, technical only on request or when the topic demands precision — security, money, data loss, irreversible actions); prefer storytelling/analogies over jargon; visualize a complex flow/algorithm with a diagram/widget instead of prose; red-team a proposal that requires judgment before agreeing (skip for simple statements/questions/greetings); stay human and empathetic, but let disagreement be substantive. Full rule: `factory/standards/conventions.md` (CONV-11).
11. **Subagent model selection (DR-111).** Scope: ad-hoc delegation only — NOT Pandacorp's own named agents (`plugin/agents/*.md` already pin `model:` in frontmatter) and NOT the build engine's own escalation (`pickWorkerModel`, DR-073/DR-108). When delegating to a subagent (the `Agent` tool, or a `Workflow`'s `agent()`) without an explicit model, calculate the tier from the SUBTASK's complexity — never default to the parent conversation's own tier (a session on Opus/Fable must not silently fan out subagents at that same tier). Haiku for mechanical/zero-judgment work, sonnet as the default floor, opus for genuine judgment/high complexity; escalate upward, never downward, if a lower tier proves inadequate. Fable is NEVER chosen automatically — only on the owner's explicit request, or after asking for confirmation first. Full rule: `factory/standards/conventions.md` (CONV-12).
