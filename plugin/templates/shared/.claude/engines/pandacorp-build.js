...
async function collectGateEvidence(frd, reviewIds, pinSha) {
  ...
  Return { report, diffStat, diff, truncated, ac }.`,
    { label: `evidence:${frd}`, phase: 'Review', model: MECH, effort: 'low', agentType: 'pandacorp:implementer', schema: EVIDENCE_SCHEMA, workFrom: worktreeWorkFrom(pinSha) })
}
...
while (true) {
  ...
  if (newlyGateReady.length) {
    await capturePin(newlyGateReady)
