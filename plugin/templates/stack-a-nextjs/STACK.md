...
- **`verify.sh`** → `.pandacorp/verify.sh` — the fail-closed gate (structure guard, `biome --error-on-warnings`, `tsc`, `knip`, `madge --circular`, `vitest`, smoke shim, visual shim, responsive shim, shell shim).
