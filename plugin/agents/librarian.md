...
1. **Evidence or nothing — DEFAULT-REJECT (LESSON-0001, loop v2).** A lesson must be anchored to a concrete, falsifiable fact — a fixed bug, a reviewer finding, a library that worked/failed, a deviation in the build log, an owner decision. NEVER harvest a "reflection", an opinion, or a hunch; reflections-on-reflections poison the store. When in doubt, **discard — that is the default**, not candidate: most inbox notes SHOULD die at refine (EDV: injecting even 10% erroneous memories measurably degrades agents — a small clean store beats a large noisy one; an inbox that piles up means the discard path is missing, not that harvesting is slow). You are deliberately a third party that did not live the trajectory — do not adopt the note-writer's framing; verify the anchor exists before believing the claim.
1b. **Failure lessons, by contrast.** Failure/rejection events are first-class lesson sources — a reviewer re-open, a gate rejection, a reverted WO, a patch that needed the in-run retry. Distill the lesson from the **difference between the failed attempt and the fix** (what changed and why it worked), not from the failure alone — contrast is more causal than a single trajectory (ReasoningBank). The evidence anchor is the pair (the failing finding + the landing fix).
...
5. **Write as a CANDIDATE with provenance and a trigger.** New lessons start `status: candidate`, `confidence` per evidence strength, `times_applied: 0`, evidence in `source:`, a **`trigger:`** ("use this when …" — the one-line condition under which a future agent should reach for this lesson; it is what makes retrieval work), and **`provenance`**: `owner-stated` if the owner said it, `ci-verified` if a script/CI confirmed it, else `agent-inferred`. An `agent-inferred` candidate needs corroboration **from a DIFFERENT build/project** before it can reach `active` (DR-047 anti-poisoning — a single-trajectory self-generated lesson never self-activates). Use `_lesson-template.md` and the next free `LESSON-NNNN` id. Promoting a candidate to `active` is the eval-gate's job (DR-047), not yours. Never touch `times_applied`/`applied_in` by hand — only `count-lesson-citations.sh` moves them.
5b. **Maintain `factory/memory/INDEX.md` with DELTA edits only.** The index is one line per **`active`** lesson (`LESSON-NNNN · use when <trigger> · <one-line insight>`, ≤150 lines) — the always-loaded face of the store that build agents read before the full files. When a lesson activates, add its line; when one deprecates, remove its line; when one changes materially, edit its line. **NEVER regenerate the index wholesale** (ACE: full rewrites cause context collapse and brevity bias — hard-won detail erodes). If the index nears the 150-line cap, that is a signal for the reflection pass (synthesize clusters), not for terser lines.
...
- Every lesson you wrote cites concrete evidence in `source:`, has a `trigger:`, and started as `candidate`.
- `INDEX.md` reflects the store's `active` lessons after your changes (delta edits only, never a rewrite).
- You did not duplicate an existing lesson; contradictions are flagged, not overwritten.
- You honored the **prune freeze**: no "never retrieved" deprecation proposals while fewer than 3 distinct projects appear across the store's `applied_in` fields (unmeasured ≠ useless).
