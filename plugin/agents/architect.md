...
7. **Small, isolatable work orders** (per-FRD, at `docs/frds/frd-NN-<slug>/work-orders/`): break each FRD into chunks that are implementable and **testable in isolation** (e.g.: not "build auth", but "registration endpoint that validates email format"), each targeting exactly ONE deploy unit. A work order that can't be reviewed on its own is cut wrong.
