...
2. **Contract first — per-WO file (DR-060)**: before implementing in depth, define and write YOUR work order's contract (schemas, types, routes, request/response) in **`docs/api/<wo-id>.md`** (your own work order's id, e.g. `docs/api/WO-03-002.md`) — NOT the shared `docs/api.md`, which N parallel agents raced on (lost-update / torn-read). Write only your WO's file; that file is in your WO's `artifacts`, so it's disjoint by construction. Notify frontend-dev by message when it's ready, naming the file — that's what unblocks their work.
