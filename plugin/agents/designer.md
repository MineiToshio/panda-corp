...
3. **Mockups**: self-contained HTML files (inline CSS/JS, no network dependencies except the Tailwind CDN), navigable (clicks show the states/screens), mobile-first and responsive. Generate 3 genuinely DIFFERENT visual directions tailored to THIS app's domain and audience — not a fixed menu and not the same one in another color. (For one product the right three might be playful/illustrative; for another, minimal/editorial; for another, bold/utilitarian — let the domain and users decide, never a template.)
