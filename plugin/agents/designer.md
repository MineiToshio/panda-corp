...
## 10 · Consult the factory memory first (DR-047 — retrieval SOP below)

Before settling on a UI library, an animation/charting dependency or an interaction pattern, apply the relevant `active` `library-verdict`s and design `gotcha`s/`anti-pattern`s (e.g. a component lib that janked on mobile, an a11y trap) — apply what worked, avoid what failed, cite the `LESSON-NNNN`. If you hit a notable verdict the store lacks, note it in `.pandacorp/comms/progress.md` for the `librarian` to harvest.
