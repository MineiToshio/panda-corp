...
const checkpoint = async (patch = {}) => { state = { ...state, ...patch, version: 2, run_id: runId, updated_at: new Date().toISOString() }; await atomic(checkpointFile, state); };
...
async function dispatch({ id, tier, prompt, units = 1, outputSchema = schema, validator = validateResult }) {
  ...
  throwIfStopping();
  if (result.code !== 0) { await event("dispatch_uncertain", { dispatch_id: id, code: result.code, signal: result.signal, timed_out: result.timedOut }); await checkpoint({ uncertain: state.inflight }); throw Object.assign(new Error(`Codex dispatch outcome is uncertain: ${id}; blind retry forbidden`), { code: result.timedOut ? "DURATION" : "UNCERTAIN" }); }
...
async function terminal(reason, detail = {}) { await checkpoint({ inflight: null, terminal_reason: reason, terminal_at: new Date().toISOString(), ...detail }); await event(reason === "complete" ? "build.complete" : "build.stopped", { reason, ...detail }); }
...
async function reconcileInflight(cp) { if (!cp?.inflight && !cp?.uncertain) return; const inflight = cp.uncertain || cp.inflight; await event("uncertain_requires_owner", { dispatch_id: inflight.id }); await needsOwner(inflight.id, "A previous Codex dispatch ended without a trustworthy terminal result. No retry was launched; inspect its durable result and git delta before deciding.", "UNCERTAIN"); }
...
try {
...
} catch (error) {
  {
    ...
    if (["uncertain", "duration"].includes(reason) && lease) { await pauseForOwner(project, lease.token, lease.epoch, { subject: state.uncertain?.id || state.inflight?.id || "uncertain-dispatch", summary: "Codex exited without a trustworthy terminal result. The executor did not retry; inspect the durable result and git delta before deciding." }).catch(() => {}); await stateCommit("chore: persist uncertain-dispatch state", null).catch(() => {}); }
    if (!state.terminal_reason) await terminal(reason, { error: error.message, code: error.code || "ERROR" }); await event("executor_stopped", { error: error.message, code: error.code || "ERROR" }); notify(`Build Codex detenido: ${reason}`); await shutdown(reason); process.exitCode = EXIT[reason] ?? 2;
  }
}
