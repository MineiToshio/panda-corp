...
6. **Reconstruct minimal AS-BUILT docs** (so downstream skills have an anchor; feature-centric, DR-049). This is reverse-documentation, clearly marked as reconstructed:
   ...
   - **Design — adopt the existing visual, don't leave it orphaned (DR-054).** If step 3 surfaced an approved visual (a `prototype/` of mockups, the live styled UI, a tokens/CSS system, a Figma), the `designer` runs the **ADOPT-VISUAL path**: extract it **faithfully** into `docs/design/design-tokens.json` + root `DESIGN.md`, record `docs/design/design-decisions.md` = "frozen FROM the approved visual at `<path>` (adopted {{DATE}})", and **stage its assets** (fonts, images, sprites — e.g. into `public/`) so the app can serve them; if the visual embeds sub-views (iframes), each embedded file is part of the contract. For each UI FRD reconstructed above, write its `fdd.md` + `mocks/` from that visual. This becomes the **binding visual source of truth** going forward — **never demote an approved prototype to a "functional-only reference"**, and never tell a later `/pandacorp:design` run to invent a fresh look over it. (This is exactly the step whose absence let Mission Control's `prototype/` sit unused while the build diverged.) If the owner says there is **no** approved visual, record that explicitly in `docs/design/design-decisions.md` (a decision, not an omission) and leave the design system to a later `/pandacorp:design` run.
