...
2. Ejecuta el scaffold del proyecto (mismos pasos del skill `scaffold`): crea `/Users/Shared/Proyectos/<slug>/` (hermana de panda-corp, nunca dentro), copia el overlay desde `${CLAUDE_PLUGIN_ROOT}/templates/shared/`, procesa los `.tpl`, crea la estructura `docs/` (frds, diseno/mockups, adr, work-orders, reviews, iteracion.md), copia la ficha a `docs/idea-origen.md`, inicializa git, crea repo privado con `gh` si está disponible (DR-010), y escribe los enlaces bidireccionales (ficha → `estado: documentada` + `proyecto:` con la ruta; fila en `fabrica/portfolio.md`).
...
7. **Iterar / reanudar (DR-032)**: si ya había PRD/FRDs, esto es una **iteración** — lee `docs/iteracion.md` (fase `producto`) y los docs existentes, refina con el feedback de Sergio (**no** regeneres desde cero ni repitas lo ya descartado) y **apenda** una entrada por ronda (qué se probó, qué rechazó y por qué, qué queda abierto). Presenta a Sergio: hipótesis de valor, FRDs de v1, qué quedó fuera.
8. **Gate de avance (DR-032)**: deja `avance_pendiente: true` en `docs/estado.yaml` y **espera el "ok, avanza"** de Sergio — re-correr `spec` mientras tanto = seguir puliendo. **NO escribas `fase: diseno` antes.** Solo cuando apruebe: `docs/estado.yaml` → `fase: diseno` + `avance_pendiente: false`, y cierra el hilo `producto` en `docs/iteracion.md`. Siguiente paso: abrir Claude Code en la carpeta del proyecto y correr `/pandacorp:design`.
