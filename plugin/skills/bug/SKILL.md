...
> **One door (DR-069):** the owner reports a bug through the single owner front door **`/pandacorp:change`** (which classifies "bug vs feature" for you so you only remember one command). `bug` is the **internal engine** that `change` and the build invoke for the bug case — it is hidden from the owner's slash menu (`user-invocable: false`); it is not an owner command.

> **Preflight (DR-045) — is this a Pandacorp project?** This skill writes one file to the project's `.pandacorp/inbox/` (capture only), so first confirm the Pandacorp marker: `.pandacorp/status.yaml` exists. If it's missing, STOP and tell the owner (in Spanish) that this folder isn't a factory project yet — `/pandacorp:adopt` brings an existing project in, `/pandacorp:spec` creates a new one. Then, if `overlay_version` in `.pandacorp/status.yaml` is behind the plugin's `OVERLAY_VERSION`, run `/pandacorp:upgrade` first (silent for compatible bumps, DR-048) — **but if a build is running (`running: true` + fresh `supervisor_heartbeat`) do NOT run `/pandacorp:upgrade` mid-build: it must never regenerate the engine/gates under a live build (its own active-build guard also refuses).** Capture only writes one file to the queue and is safe on an older overlay, so file the bug anyway — this gates only the optional upgrade side-step; capture itself stays build-detection-free (the DR-069 safety above).
...
2. **Write the change-request — the SAME card `/pandacorp:change` step 4 writes, with `type: bug`.** Don't re-specify the format here: follow `change` step 4 (`.pandacorp/inbox/changes/<slug>.md` from `${CLAUDE_PLUGIN_ROOT}/templates/docs/change-request-template.md`, short English slug, + its row in `.pandacorp/inbox/changes/README.md`), setting `type: bug`, `class` = `expedite` if it blocks a flow else `standard`, and `status: ready` (an owner-described bug is born `ready` so the drain picks it up). The **owner sets only `draft`/`ready`**; the engine then manages the transient `building` (the change's FRDs are in flight — not re-drained) and terminal `done` (archived), so the full queue enum is **`draft | ready | building | done`** (DR-069; `pending` is not a queue status). The bug's own capture fields go in the **body (español)**: **pasos para reproducir · resultado esperado · resultado actual**.
3. **Confirm** to the owner: "noted in the inbox; `/implement` will take it at its next safe point." If there is NO build running and they want to fix it now, tell them to run `/pandacorp:implement` (which empties the inbox).
