...
3. **Granularity — COARSE: one work order = one cohesive view / page / capability** (e.g. "order detail view + payments panel + action menu"), NOT one atomic component. The build engine reviews and tests **per FRD**, so each work order must carry enough context for an agent to build the whole slice end-to-end. If it's too big to review on its own, split; if it's smaller than a usable slice, merge. Typical: a handful of coarse work orders per FRD (think PandaTrack: ~one per view), not dozens of tiny ones.
