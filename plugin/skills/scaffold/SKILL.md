...
3. **Copy the Pandacorp overlay** from the plugin's templates:
   ...
   - The overlay already includes `.claude/workflows/pandacorp-build.js` (the build engine that `implement` launches), `.claude/settings.json`, and the `.pandacorp/` integration layer (`guide.md`, `status.yaml`, `README.md`, `comms/`).
