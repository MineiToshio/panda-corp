...
4. **Record the outcome** in `.pandacorp/inbox/decisions.md` — never delete a block, only mark it. "Mark it" means **edit that decision's OWN block in place** (its heading's status word + a `- **Estado:**` line) — never just append a new sibling entry narrating the resolution while leaving the original heading untouched (that leaves the decision mechanically "pending" forever; a real incident, 2026-06-30). A new entry with more context is welcome IN ADDITION, never INSTEAD:
