...
5. When every **Done when** criterion is objectively true:
   ...
   - `plugin/` changed → bump `plugin/runtime/plugin-metadata.json`'s `version` per semver (CLAUDE.md's PATCH/MINOR/MAJOR rule — this is the SOURCE, DR-113; the two `plugin.json` manifests are generated projections and must NEVER be hand-edited), then run `node plugin/scripts/generate-plugin-manifests.mjs` to regenerate both manifests, commit the source AND the regenerated manifests together, add the entry to `plugin/docs/decision-log.md`, and run `claude plugin validate plugin/`.
...
Default: run the recipe above directly, in this conversation. **Escalate up only (DR-111/CONV-12), never down-tier:** if the item's Fix plan computes to a HIGHER model tier (per the CONV-12 rubric above) than the current conversation's own model, dispatch the **Implement & close** recipe to a right-sized subagent at that higher tier instead of running it inline — e.g. an opus-tier item worked from a sonnet conversation. If the item's tier is the same as or lower than the conversation's own, stay inline; a lower-tier item is never worth downgrading the conversation for.
...
2. Internally the workflow: **Scan** — one haiku agent reads every `factory/backlog/BL-*.md` frontmatter and sizes each item's model tier (see the rubric below — the scan agent cites it directly), keeping only `open`/`doing` items, honoring `args.items`, capping at `args.maxItems` and logging anything dropped (no silent caps). **Implement** — `parallel()` dispatches one agent per item, model = that item's tier, isolated in its own `git worktree` (`.claude/worktrees/bl-<id>`, branch `bl/<id>`); each implements + proves + closes the item's own frontmatter + commits **inside its worktree only**, never touching `main`, and reports `done` or `blocked` (blocked items are never retried automatically). **Merge** — a plain serialized loop, one sonnet agent per successful item, in return order: rebase onto `main`, `ff-only` merge (or a resolved merge using the named hotspots — `plugin/runtime/plugin-metadata.json`'s `version` (the SOURCE, DR-113) keeps the HIGHER, then `node plugin/scripts/generate-plugin-manifests.mjs` regenerates both `plugin.json` manifests from the resolved source so they are never hand-resolved themselves, `decision-log.md` keeps BOTH entries most-recent-on-top, `factory/backlog/README.md`'s count gets recounted from files — and if BOTH merged branches independently bumped `plugin-metadata.json` from the same stale base, that's a **collapsed bump**: only one increment survives even though two items each thought they were the one bumping it; the merge step must NOT silently accept the tie — it flags it LOUDLY in the returned report for your reconciliation, interim honesty fix per BL-0025, which still tracks the real sequential allocator), then `bash plugin/scripts/validate-backlog.sh` — red reverts that merge and marks the item blocked; green removes the worktree + branch before the next merge. **Report** — the workflow returns `{ done: [...], blocked: [...] }` (a `done` entry can carry a `[FLAG: ...]` suffix for a collapsed bump) and does nothing else (no push, no notification).
...
**Model-tier rubric the scan agent applies:** the full rubric (haiku/sonnet/opus tiers, Fable never automatic) is `factory/standards/conventions.md`'s **CONV-12** (DR-111) — the scan agent applies it to the item's `severity` and how much its **Fix plan** actually touches. Don't re-derive it here; if CONV-12 changes, this pointer follows automatically.
