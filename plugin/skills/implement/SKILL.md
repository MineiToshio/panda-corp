...
`$ARGUMENTS` optional: a **mode** (`pro` | `balanced` | `powerful` | `deep`; default `balanced`); `maxFrds` (**opt-in** cap on features **processed** per run — built + blocked + **reopened**, so reopens count — for **supervised TEST runs**); and **`maxSpend`** (an absolute output-token ceiling — the real guardrail for **overnight runs**: the build stops at a safe point when reached, even without a `+Nk` turn directive).
