...
2. **Copy de landing + telemetría** (en paralelo a la auditoría):
   - `copywriter`: escribe el copy de la landing del MVP (headline, propuesta de valor, beneficios, CTA, FAQ) con `title`/meta description legibles y orientados a la intención de búsqueda (colabora con `fabrica/estandares/seo-i18n.md`). Ningún claim sin respaldo.
...
5. **GATE HUMANO — PRODUCCIÓN (DR-004)**: presenta al dueño el resumen (URL de staging para que pruebe, resultado de auditoría, costos al activar producción si los hay — DR-005). **Espera su aprobación explícita. Sin excepciones.**
6. **Deploy a producción** (agente `devops`) tras aprobación + verificación post-deploy (smoke test en prod) + plan de rollback listo.
7. **Cierra** (documentación viva, DR-018): tag de versión (semver), **changelog auto-generado desde Conventional Commits**, ADRs al día (proponer uno si el release incluyó un cambio arquitectónico no registrado), README/docs de usuario actualizados, `docs/estado.yaml` → `fase: operacion`, ficha de la idea en la fábrica → `estado: lanzada`, fila del portfolio actualizada.
...
- Si el dueño no aprueba, documenta sus razones en `docs/estado.yaml` y genera los work orders de ajuste.
