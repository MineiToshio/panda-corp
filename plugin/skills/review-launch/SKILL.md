...
The back half of the economic arc (DR-043). The factory can ship and instrument; this skill makes it **learn** whether what it shipped actually works, and decide where to put effort next. Runs IN the project; precondition `phase: operation` in `docs/status.yaml` (a version is live with telemetry already verified by `release`).

`$ARGUMENTS` optional: a window (e.g. `30d`, `60d`); default = the window from the PRD's kill-signals.

## Steps

1. **Read the targets.** From `docs/prd.md`: the value hypothesis, the **activation milestone**, the success metrics and the **kill-signals with their numeric thresholds** (DR-043). From `docs/idea-origin.md`: the `return_type` — it decides which metric matters.
