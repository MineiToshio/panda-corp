...
3. **Regenerate the managed layer (non-destructive).**
   ...
   - **Gate-config conformance check (DR-059)** — the meta-fix that catches silent enforcement drift. The gate config propagates VERBATIM, exactly like `docs/rules/`. Read the project's stack from `docs/product/architecture.md` and resolve `${CLAUDE_PLUGIN_ROOT}/templates/stack-<chosen>/`. **Diff** the project's `verify.sh` (`.pandacorp/verify.sh`), `biome.json` and `knip.json` (project root) against the stack templates; on **any drift** (or a missing file) **regenerate it from the template, overwriting the project's copy** (the plugin is the source of truth) and note the drift in the report — a project's enforcement can never silently fall behind the standard (this conformance check is what would have caught the audit: smoke/visual gates, `knip`, the `react`/`next`/`test` domains and `complexity: error` all silently OFF, with nothing checking the wiring). A pre-DR-059 project simply gets these three files installed for the first time. Leave untouched any project-specific lint/knip config the owner added in a **separate** file (only the canonical ones are conformance-checked).
   - **Toolchain conformance check (DR-059, extends the gate-config check)** — a byte-perfect `verify.sh` still goes RED at cold start if the tools it calls aren't installed: a missing `knip` or `playwright` fails the gate even with a canonical config. Conformance therefore covers the **toolchain**, not just the config files — this is what makes the fail-closed gate actually *runnable*. After the config diff, read the project's `package.json` and reconcile it against what the canonical `verify.sh` requires (resolve from the same `stack-<chosen>/` template); on any gap, install/fix it and note it in the report:
     - **Required devDependencies** — `knip`, `madge`, `@playwright/test`, `vitest`, `@biomejs/biome`. Any missing from `devDependencies` → install (`@latest` for a current project; framework-major-compatible for brownfield, DR-052).
     - **Required `package.json` scripts** the canonical `verify.sh` invokes — `test:smoke` (`playwright test e2e/smoke.spec.ts`, DR-055) and `test:visual` (`playwright test e2e/visual.spec.ts`, DR-056). Add any that are absent.
...
- **Gate config is conformance-checked, not merged (DR-059)**: `verify.sh`/`biome.json`/`knip.json` are canonical — the plugin is the source of truth and any project drift is overwritten from the template, exactly like `docs/rules/`. The owner's project-specific lint/knip tweaks belong in a separate file, which is left untouched.
- **Conformance covers the toolchain, not just the config (DR-059)**: a byte-perfect `verify.sh` is useless if `knip`/`playwright` aren't installed — the check also reconciles the required devDependencies, the `test:smoke`/`test:visual` scripts and a strict `tsconfig`, so the fail-closed gate is actually runnable at cold start.
