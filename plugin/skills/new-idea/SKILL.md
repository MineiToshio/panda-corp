...
5. **Create the card** `factory/ideas/<slug-in-english>.md` with complete frontmatter (includes `project_type`, `profile_alignment` and `return_type`), `status: discovered`, `created:` with today's date. If an exploration draft existed (`factory/ideas/_drafts/<slug>.md`), **delete it** — the card now replaces it.
