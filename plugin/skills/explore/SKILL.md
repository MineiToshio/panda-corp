...
Mira `$ARGUMENTS` y la base de ideas (`factory/ideas/`) para ubicar de dónde arrancas:
- **Idea nueva y difusa** → arrancas de cero, pensando con el dueño.
- **Retomas una exploración** → si hay un borrador en `factory/ideas/_drafts/<slug>.md`, léelo y continúa el hilo (no empieces de nuevo).
- **Desarrollas una ficha que ya existe** (la creó `discover` o `new-idea`, sigue en «Descubierta») → lee la ficha completa (`factory/ideas/<slug>.md`) y su borrador si lo hay, y sigues desarrollándola: profundizas el problema, validas con evidencia, afinas la solución. Sigues sin escribir en la ficha (es el gate); al cristalizar, `new-idea` **actualiza esa misma ficha**, no duplica.
...
- **Conecta con lo que ya hay.** Lee los frontmatter de `factory/ideas/*.md` (en la raíz de la fábrica) y, si la conversación roza una idea existente, dilo ("esto se parece a tu idea X — ¿la extendemos o es distinta?").
...
Para poder retomar desde otra sesión, otra computadora o el celular, la exploración deja un **borrador durable** (no es una ficha, no aparece en el tablero): `factory/ideas/_drafts/<slug>.md`.
