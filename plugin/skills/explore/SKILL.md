...
- **Look through the factory's lens.** Filter everything by what this factory can build: implementable with golden paths by one person in the **micro/small** band (prefer a cotizapdf-sized micro-app; a large idea only if clearly worth it), clear monetization path OR high personal value to the owner, without heavy regulatory requirements (health, regulated finance). If a nice idea clashes with this, say so.
- **Push toward founder-fit (so the idea is HIS).** Steer toward problems the owner or his niche **already live** (Paul Graham's traits) — anchored to his channel/collecting world, not a generic market. An idea that doesn't nacer from his life/channel gets questioned with an argument, not just polished.
- **Surface the hard gaps EARLY** (so crystallization doesn't inherit a fatal flaw): if the app would emit a judgment, ask *"where would it get the truth?"* (ground-truth); flag any legal risk; and gauge buildability (micro/small/large). Better to hit these in conversation than in the card.
