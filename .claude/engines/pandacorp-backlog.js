...
5. When every Done when criterion is objectively true, in the worktree:
   ...
   - If the fix touched plugin/: bump plugin/runtime/plugin-metadata.json's version per semver (PATCH/fix, MINOR/capability, MAJOR/breaking) — this is the SOURCE (DR-113); NEVER hand-edit plugin/.claude-plugin/plugin.json or plugin/.codex-plugin/plugin.json, they are generated projections and the derived-drift Stop gate REDs on a hand-edit. Then run \`node plugin/scripts/generate-plugin-manifests.mjs\` to regenerate both manifests, commit the source AND the regenerated manifests together, add the plugin/docs/decision-log.md entry (most-recent-on-top), and run \`claude plugin validate plugin/\` from inside the worktree.
...
3. From the MAIN checkout: \`git -C ${FACTORY_ROOT} merge --ff-only ${item.branch}\` when possible. If ff-only fails, do a normal merge and resolve conflicts by hand using these named hotspots:
   - plugin/runtime/plugin-metadata.json version field (the SOURCE, DR-113): keep the HIGHER of the two versions, never blindly pick one side. After resolving it, run \`node plugin/scripts/generate-plugin-manifests.mjs\` from ${FACTORY_ROOT} to regenerate plugin/.claude-plugin/plugin.json and plugin/.codex-plugin/plugin.json from the resolved source — NEVER hand-resolve those two manifests directly, they are generated projections and the derived-drift Stop gate REDs on a hand-edit. If this was a genuine CONFLICT on the version field (both sides independently bumped it from the same stale base, not just one side changing it) — a COLLAPSED BUMP: only one increment survives even though two items each thought they were the one bumping from a stable base. Do NOT try to compute a combined/correct bump yourself (that is BL-0025's still-open sequential-allocator work) — just keep the higher value, regenerate the manifests from it, AND set the returned \`reason\` field to a one-line flag even though the merge succeeds, e.g. \`"COLLAPSED-VERSION-BUMP: ${item.id} and another item both bumped plugin-metadata.json — verify the surviving version covers both changes' severity"\`, so the owner can reconcile manually.
