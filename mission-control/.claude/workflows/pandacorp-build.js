export const meta = {
  ...
  description:
    "Pandacorp build engine: waves of work orders (build → review → verify) with the factory subagents, fail-closed gates and one commit per safe point. Resumable and app-agnostic — it works over the work-order queue and the project verify.sh, not over anything hardcoded in the product.",
  phases: [{ title: "Plan" }, { title: "Build" }, { title: "Review" }, { title: "Verify" }],
};
      ...
      items: {
        ...
        properties: {
          ...
          deps: {
            type: "array",
            items: { type: "string" },
            description: "ids that must be green first (may live in another FRD)",
          },
          frd: {
            type: "string",
            description: "the FRD folder this WO lives under, e.g. frd-03-<slug>",
          },
        },
      },
    },
...
log(
  `${plan.workOrders.length} pending work orders · stack ${plan.stack}${plan.hasFrontend ? " (web)" : ""}`,
);
...
async function build(wo) {
  ...
  if (plan.trivial || !P.split) {
    await agent(
      `${EMIT("implementer", wo.id)}Fully implement work order ${wo.id} with TDD (RED→GREEN→refactor), anchored in the EARS criteria of FRD ${wo.frd || ""} and in bugs from .pandacorp/comms/progress.md: ${wo.summary}. Write the critical context to files.`,
      {
        label: `build:${wo.id}`,
        phase: "Build",
        model: P.worker,
        agentType: "pandacorp:implementer",
        schema: STAGE_SCHEMA,
      },
    );
    return;
  ...
  await agent(
    `${EMIT("test-writer", wo.id)}Write the acceptance tests (RED phase) for work order ${wo.id} anchored in the EARS criteria of FRD ${wo.frd || ""}: ${wo.summary}. Do not write production code.`,
    { label: `test:${wo.id}`, phase: "Build", model: P.worker, agentType: "pandacorp:test-writer" },
  );
  await agent(
    `${EMIT("backend-dev", wo.id)}Implement the backend of work order ${wo.id} with TDD until the tests are green: ${wo.summary}. Publish the API contract in docs/api.md.`,
    { label: `be:${wo.id}`, phase: "Build", model: P.worker, agentType: "pandacorp:backend-dev" },
  );
  if (plan.hasFrontend) {
    await agent(
      `${EMIT("frontend-dev", wo.id)}Implement the UI of work order ${wo.id} using ONLY design tokens and consuming the contract in docs/api.md: ${wo.summary}.`,
      {
        label: `fe:${wo.id}`,
        phase: "Build",
        model: P.worker,
        agentType: "pandacorp:frontend-dev",
      },
    );
  }
}
  ...
  if (P.deepReview) {
    ...
    const verdicts = await parallel(
      lenses.map(
        (L) => () =>
          agent(
            `${EMIT("reviewer", wo.id)}Review work order ${wo.id} through the ${L} lens. Re-run the evidence yourself and write adversarial tests the implementer did not see (anchored in EARS and real bugs). Return ok=false if you find a real defect.`,
            {
              label: `rev:${wo.id}:${L}`,
              phase: "Review",
              model: P.judge,
              agentType: "pandacorp:reviewer",
              schema: STAGE_SCHEMA,
            },
          ),
      ),
    );
    if (verdicts.filter(Boolean).some((v) => v && v.ok === false))
      throw new Error("REVIEW_REJECT: lens in red");
    return;
  }
  const v = await agent(
    `${EMIT("reviewer", wo.id)}Review work order ${wo.id} with your 3 lenses (correctness/security/quality). Re-run tests/lint/typecheck yourself (do not trust the self-report) and write adversarial tests the implementer did not see. Return ok=false with the reason if something does not pass.`,
    {
      label: `rev:${wo.id}`,
      phase: "Review",
      model: P.judge,
...
async function runWO(wo) {
  try {
    ...
    await agent(
      `Freeze-on-red for work order ${wo.id}: do NOT commit anything broken. Leave HEAD at last_green_sha, mark work order ${wo.id} as BLOCKED in .pandacorp/status.yaml with the reason, and emit a notification to the owner (PushNotification / Notification hook). Do not touch other work orders.`,
      {
        label: `freeze:${wo.id}`,
        phase: "Verify",
        model: P.worker,
...
while (pending.size > 0) {
  ...
  for (const wo of dead) {
    blocked.add(wo.id);
    pending.delete(wo.id);
    log(`⊘ ${wo.id} skipped (depends on a blocked one)`);
  }
  if (ready.length === 0) {
    if (pending.size > 0)
      log(`⚠ ${pending.size} work orders unresolved (circular or blocked deps)`);
    break;
  }
  if (budget.total && budget.remaining() < 60000) {
    log("Circuit breaker: low budget, stopping at a safe point");
    break;
  }

  const wave = ready.slice(0, P.wave);
  const results =
    P.wave === 1 ? [await runWO(wave[0])] : await parallel(wave.map((wo) => () => runWO(wo)));

  for (let i = 0; i < wave.length; i++) {
    pending.delete(wave[i].id);
    if (results[i] && results[i].green) {
      done.add(wave[i].id);
      built.push(wave[i].id);
...
if (built.length > 0 && blocked.size === 0) {
  await agent(
    `All work orders closed green. Run the full suite + e2e of the critical flows and kill the test dev servers with TaskStop. If everything stays green, set .pandacorp/status.yaml phase: release and running: false. Summarize what was built and the evidence.`,
    { label: "close-out", phase: "Verify", model: P.judge, agentType: "pandacorp:reviewer" },
  );
} else if (blocked.size > 0) {
  log(
    `Build stopped with ${blocked.size} blocked: ${[...blocked].join(", ")}. Check .pandacorp/inbox/decisions.md / .pandacorp/inbox/bugs/ and re-run implement.`,
  );
}
