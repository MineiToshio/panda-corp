 ...
 *
 * Tests the server component that calls readEvents and builds the party snapshot.
 *
 * Traceability:
...
import { FIXTURE_EVENTS_EMPTY_NDJSON, FIXTURE_EVENTS_NDJSON } from "@/tests/fixtures/index";
import { PartyTab } from "./PartyTab";

// ---------------------------------------------------------------------------
// AC-06-010.1 — empty state when no events
// ---------------------------------------------------------------------------
...
describe("frd-06: PartyTab — reads events, no Claude call (AC-06-008.1)", () => {
  it("frd-06: WHEN the NDJSON has 10 events THEN renders the event feed (active)", () => {
    render(<PartyTab eventsPath={FIXTURE_EVENTS_NDJSON} />);
    expect(screen.getByTestId("event-feed")).toBeDefined();
  });

  it("frd-06: WHEN the NDJSON has events THEN does NOT render the empty state", () => {
    render(<PartyTab eventsPath={FIXTURE_EVENTS_NDJSON} />);
...
describe("frd-06: PartyTab — event cap (WO-06-005 scope)", () => {
  it("frd-06: WHEN cap is set to 3 and fixture has 10 events THEN only 3 rows render", () => {
    render(<PartyTab eventsPath={FIXTURE_EVENTS_NDJSON} cap={3} />);
...
describe("frd-06: PartyTab — lastEventAt indicator", () => {
  it("frd-06: WHEN events present THEN renders the Live indicator", () => {
    render(<PartyTab eventsPath={FIXTURE_EVENTS_NDJSON} />);
