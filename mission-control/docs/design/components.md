...
| `KanbanColumn` | Fixed-width WO column (`.col`, 224px, header label + count, vertical-scroll body for WO cards) | `src/components/core/KanbanColumn/KanbanColumn.tsx` | `label`, `count`, `children` | **real** (WO-13-008) |
...
| `ObservabilidadTab` | The **Observabilidad** project tab (sibling of Party) — local `SectionHead` strip + the **Línea de tiempo ↔ DAG** `Tabs` toggle over the SAME work orders; points operators to Party for live agents (FRD-12) | `src/app/projects/[slug]/_observability/ObservabilidadTab/ObservabilidadTab.tsx` | `view` (timeline/dag); reads WO set | **planned** (prototype `observabilidadBody`, ~L1214) |
| `TimelineView` | WO→tasks→actions Gantt-style timeline: duration bars positioned by start/dur, nested faint task bars, time axis, jump-to-first-error note (FRD-12) | `src/app/projects/[slug]/_observability/TimelineView/TimelineView.tsx` | `workOrders`, total duration; `tabular-nums` | **planned** (prototype `bTimeline`, ~L1156) |
| `WoDag` | Work-order **dependency graph** (Dagre, ~39KB) — bezier-edge DAG with **chain-highlight** (upstream+downstream, dims the rest), **jump-to-first-error**, **follow-active-step** (FRD-12) | `src/app/projects/[slug]/_observability/WoDag/WoDag.tsx` | `workOrders`, `activeNode`, `follow`; SVG nodes + state dots | **planned** (prototype `bDag`/`bDagChain`, ~L1169/~L1154) |
...
| Name | Purpose | Path | Key props / variants | Status |
|---|---|---|---|---|
| `Room` | Pixel-art zone room (Forge/Tribunal/Bóveda; the 6 Campaña phases) + label + count/badge | `src/components/modules/party/Room/Room.tsx` | `zone`, `state` (cool/hot/active/done/locked), `label`, `count` | **planned** (composed in `FraguaScene`/`CampaignPipeline` today) |
| `StoneBridge` | PNG stone connector between rooms; active one **flows** the deliverable | `src/components/modules/party/StoneBridge/StoneBridge.tsx` | `orientation` (h/v), `flow`, `pos` | **planned** |
| `FlowStrip` | Always-visible whole-pipeline strip lighting the active beat(s) + hover tooltips (8 beats) | `src/components/modules/party/FlowStrip/FlowStrip.tsx` | `beats`, `activeKeys`; hidden in embed | **planned** |
| `PowerOffOverlay` | Factory-off treatment derived from real state (desaturate map, tidy sprites, "Fábrica apagada") | `src/components/modules/party/PowerOffOverlay/PowerOffOverlay.tsx` | derived `off`; today `PartyEmptyState` | **planned** (today: `PartyEmptyState/PartyEmptyState.tsx` — **real**) |
| `MissionBar` | Real mission surface (`.quest`): FRD pips + global WO counter + **effort as read-only data** (DR-061) | `src/components/modules/party/MissionBar/MissionBar.tsx` | `frdPips`, `done`, `total`, `effort` (read-only) | **planned** |
| `DemoControls` | **DR-061** wrapper — dashed-border + `SOLO DEMO` tag + one-line note for preview-only controls; never ships read-only | `src/components/modules/party/DemoControls/DemoControls.tsx` | `note`, children; demo-only | **planned** |
| `AgentSprite` | 52/54/58px pixel implementer/specialist sprite (1 = 1 running WO) with halo, progress, tag | `src/components/modules/party/AgentSprite/AgentSprite.tsx` | `role`, `size`, `state` (work/review/carry/vault/split/idle/walking/say-on) | **planned** (in `FraguaScene`) |
| `JudgeSprite` | The single reviewer per FRD — dim until the gate opens, then paces the Tribunal | `src/components/modules/party/JudgeSprite/JudgeSprite.tsx` | `active`, `judgingTarget` | **planned** |
| `SpeechBubble` | Agent speech bubble (`sayin`; even columns talk higher) | `src/components/modules/party/SpeechBubble/SpeechBubble.tsx` | `text`, `raised`, via `say-on` | **planned** |
| `Tooltip` | Hover tooltip for a sprite/beat (WO id+title; phase role+name) | `src/components/modules/party/Tooltip/Tooltip.tsx` | `content`, anchor | **planned** |
| `Parchment` | The travelling 📜 = the real Status-Note hand-off between dependent WOs | `src/components/modules/party/Parchment/Parchment.tsx` | `from`, `to` (Build Plan dependency) | **planned** |
