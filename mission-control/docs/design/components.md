...
| `GuildBar` | Guild/status top bar (`topbar`): logo + title + NV level pill + guild title + inline `XpBar` (FRD-09) | `src/components/modules/GuildBar/GuildBar.tsx` | `outcomes: GuildOutcomes`; derives level/title/xp via `computeGuildLevel`; compact `XpBar` (size="compact") | **real** (WO-09-003) |
