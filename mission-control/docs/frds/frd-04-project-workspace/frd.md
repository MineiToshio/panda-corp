...
- WHEN there are pending decisions, it SHALL highlight them, EACH labeled with its stable `id` (`<date>-<n>` / `legacy-<n>`, derived from its heading — never invented, never reused) so the owner can reference one specific decision; the **per-card command SHALL be `/pandacorp:decide <id>`** — scoped to that decision, not the bare un-scoped form (which would walk every pending one). WHERE a pending decision carries an AI recommendation, the Summary SHALL offer an **"Aprobar la recomendación" one-click** that copies `/pandacorp:decide <id> "Aprobado: <recommendation>"` to the clipboard (it only copies the command — the app never writes or calls Claude).
