...
- The **Commands** tab SHALL show the commands relevant to the stage and the **build mode selector** ([FRD-11](../frd-11-build-modes/frd.md)). In the `implementation` phase it SHALL show five rows in this order: (1) `/pandacorp:implement` — full build, (2) `/pandacorp:implement <frd>` — partial build targeting a single FRD, (3) `/pandacorp:implement change:<slug>` — process a change from the queue and build only the affected FRDs, (4) `/pandacorp:release` — launch when all work orders are done, (5) `/pandacorp:iterate` — add or adjust. In the `release` phase it SHALL show five rows: (1) `/pandacorp:iterate` — add/adjust/fix, (2) `/pandacorp:implement` — build all pending FRDs generated by an iterate, (3) `/pandacorp:implement <frd>` — partial build by FRD, (4) `/pandacorp:implement change:<slug>` — process a change from the queue, (5) `/pandacorp:new-version` — start a new major milestone.
