...
- The **Rangos** tab SHALL show the **40-rung guild rank ladder** (FRD-09) as a vertical climb: every rank (icon + `NV n` + Spanish name + XP threshold), the **current rank highlighted** ("ESTÁS AQUÍ" + a bar to the next), ranks already earned marked done, and ranks ahead dimmed/locked (fading further toward the summit, so the top reads as deliberately distant).
