...
- The **Rangos** tab SHALL show the **40-rung guild rank ladder** (FRD-09) as a vertical climb: every rank (emblem + grade badge + Spanish name + its **level band** `Nv min–max`, open-ended `Nv N+` at the top), the **current rank highlighted** ("ESTÁS AQUÍ" + a bar of progress to the NEXT rank), ranks already earned marked done, and ranks ahead dimmed/locked (fading further toward the summit, so the top reads as deliberately distant). The rank is the BAND that contains the guild's granular level (FRD-09 `rankForLevel`), not a 1:1 level.
