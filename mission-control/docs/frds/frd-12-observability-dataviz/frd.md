...
- **AC-12-004.1b (FRD swimlanes — readable grouping, dependency-ordered)** — The DAG SHALL group nodes into **per-FRD swimlanes**: each FRD is a labeled horizontal band (`FRD-NN · Name`); within a band, its work orders are laid out left-to-right by the band's **internal** dependency depth (only intra-FRD edges rank the columns), so a dependency-free work order piles only within its own small band — never into one giant global first column. **Cross-FRD edges** render with a **distinct (dashed)** style so they read as jumps between bands. The bands SHALL be ordered by **FRD-level dependency** (a DFS pre-order over the FRD super-graph — each dependency chain contiguous, a prerequisite FRD immediately followed by its dependents — with a numeric-FRD tie-break), so **related FRDs sit adjacent** (e.g. FRD-18, which depends on FRD-01, lands directly under FRD-01) and cross-FRD edges stay short. The ordering SHALL be **deterministic** (stable across server reads + live updates) and **cycle-safe** (a cross-FRD cycle falls back to numeric order). Decided by red-team over a force-directed/Obsidian layout, which was rejected for non-determinism + loss of the directional reading. (This replaces both the single global layered layout — whose rank-0 column collapsed every root together — and the earlier numeric band order.)
