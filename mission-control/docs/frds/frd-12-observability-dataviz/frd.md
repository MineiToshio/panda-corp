...
- **AC-12-004.1b (FRD swimlanes — readable grouping)** — The DAG SHALL group nodes into **per-FRD swimlanes**: each FRD is a labeled horizontal band (`FRD-NN · Name`), ordered top-to-bottom by FRD number; within a band, its work orders are laid out left-to-right by the band's **internal** dependency depth (only intra-FRD edges rank the columns), so a dependency-free work order piles only within its own small band — never into one giant global first column. **Cross-FRD edges** render with a **distinct (dashed)** style so they read as jumps between bands. (This replaces the single global layered layout whose rank-0 column collapsed every root work order together.)
