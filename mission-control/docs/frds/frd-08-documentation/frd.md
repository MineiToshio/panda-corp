...
- WHILE Codex `implement` remains `FALLBACK`, THE multi-runtime Concept SHALL state that normal build writes fail closed, SHALL identify R10 Stage 2 and R11 `LIVE_OVERNIGHT` as distinct one-shot disposable certification exceptions, and SHALL NOT present either exception as normal-project permission or promotion.
