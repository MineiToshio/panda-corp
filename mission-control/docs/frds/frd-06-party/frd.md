...
- AC-06-001.6: THE per-work-order **room placement** (forge / Tribunal / Bóveda) SHALL agree with the **authoritative work-order state** the Work Orders board shows — the frontmatter `implementation_status` read by `listWorkOrders` (the SAME source, DR-092 single source for derived state) — NOT a second derivation. WHERE the board shows a work order as `IN_REVIEW`, THE Party SHALL place its sprite in the **Tribunal** (`in_review`); `VERIFIED` → **Bóveda** (`verified`); `BLOCKED`/`fail` → `blocked`; otherwise → the forge (`building`). The event stream still drives **liveness/animation** (which sprite is alive, the walk), but the **frontmatter decides which room**. **(IMPLEMENTED, 2026-06-30)** — previously every running sprite was hardcoded `building`, derived only from events, so the Party showed a WO "construyéndose" while the Work Orders tab already showed it `REVIEW`. `toFraguaSnapshot` now takes an authoritative `woStates` map (id → state, via `toWoStates(listWorkOrders())`); the RSC passes it at render and carries it into the live re-derive so SSE event frames don't revert the room. **Bound:** `woStates` is captured at each Party-tab render (refreshes on tab switch / reload); a state transition occurring with the tab already open reflects on the next render, not mid-frame (the SSE frame carries events, not frontmatter). Carrying `woStates` in the live frame is a follow-up.
