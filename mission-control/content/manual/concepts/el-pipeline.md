...
Se documenta la arquitectura técnica de la plataforma (`docs/product/architecture.md`) y el blueprint de cada FRD (`docs/frds/frd-NN-<slug>/blueprint.md`). Se generan las **work orders** que dividen la construcción en rebanadas cohesivas (COARSE, con banda **calibrada con datos de builds reales** — DR-100, 2026-07-01: objetivo ~25–50 min de build / ~1.5–4k LOC por WO; **techo** ~4k LOC, por encima los rechazos del gate escalan superlinealmente; **piso** ~20 min, porque el gate por FRD cuesta ~9 min fijos y una feature diminuta pagaría más gate que build — se fusiona con la feature que extiende). El Build Plan del blueprint lleva una **tabla DAG obligatoria** (WO · dependencias · artifacts · foundation · paralelo-con) que el motor parsea directamente, y el readiness gate verifica además que los `artifacts` declarados sean **completos** (no solo disjuntos): con las oleadas globales, una omisión puede chocar entre FRDs.

Antes de que las work orders pasen a `ACTIVE`, el plan cruza **tres gates independientes que corren EN PARALELO** — cada uno por un agente FRESCO en su propio contexto y de tier JUDGE (opus, DR-111): el **readiness gate** (DR-100) —cada requisito mapea a un componente, **cada criterio de aceptación lo cubre exactamente una work order**, el modelo de datos no tiene huecos (`TBD`), el grafo de dependencias es acíclico, la foundation está completa y **no queda ninguna pregunta abierta sin resolver** (`[NEEDS CLARIFICATION]` bloquea)—, el **grounding gate** (DR-102) que valida cada afirmación concreta (rutas, APIs del framework, versiones) contra el scaffold realmente instalado, y el **contradiction gate** (DR-116) que asegura que el set no se contradice a sí mismo ni a los FRDs que implementa. El flip a `ACTIVE` espera a que **los tres** salgan en verde; con cualquier bloqueo, los docs se quedan en `DRAFT`. Un blueprint con agujeros produce work orders ambiguas, así que la cohesión se **verifica**, no se confía.
...
Antes de la fase *product*, una idea nace en el tablero. `/pandacorp:discover` sale a internet a **encontrar** oportunidades en **dos fases**: un barrido ancho y barato por lentes de caza rotativos (extensiones que arreglan quejas de apps conocidas · tus otras identidades · own-itch · challengers AI-first/unbundling de SaaS exitosos) que te presenta 8-12 **teasers** de una línea con evidencia de fuentes verificadas; tú eliges los que te chispean, y solo esos pasan por los gates de calidad (dedup por problema, founder-fit, painkiller, cubeta de construibilidad micro/pequeña/grande, **viabilidad técnica** —¿se puede conseguir el dato/capacidad crítica?—, fuente-de-la-verdad + riesgo legal) y un red team, para documentarse como cards. `/pandacorp:new-idea` hace lo mismo cuando **tú** traes la idea: la documenta y propone cómo mejorarla, sin venderte nada.
