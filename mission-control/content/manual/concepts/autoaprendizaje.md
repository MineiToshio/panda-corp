...
Una lección llega a `promotion: proposed` por dos vías: el **escalador automático** (≥3 proyectos la citan → el script la auto-encola) o el barrido manual de `/pandacorp:memory review`. El buzón de **Propuestas** (FRD-17) la muestra y `/pandacorp:learn` la aplica con gate tierado (DR-047): un `SHOULD` con verificador determinista puede aplicarse solo (notificando al dueño); un `MUST`, una regla o un skill siempre esperan el gate humano. `/pandacorp:learn` también puede promoverla **directamente dentro de la misma conversación**, sin pasar antes por `promotion: proposed`, cuando el dueño la aprueba ahí mismo (así llegó LESSON-0147 a PROMPT-8, 2026-07-15). Tras promover, por cualquiera de las dos rutas: back-link a la lección, `promotion: approved`/`rejected` **y, si se aprobó, `status: active`** (BL-0088) — la aprobación del dueño ya es corroboración más fuerte que el umbral cruzado de proyectos del eval-gate, así que una lección `promotion: approved` nunca queda en `status: candidate`, invisible para `INDEX.md`; la lección nunca se borra.
