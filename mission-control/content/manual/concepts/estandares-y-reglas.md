...
Desde 2026-07-04 la fábrica comparte su know-how con **cualquier runtime de coding** que hable los estándares abiertos AGENTS.md y Agent Skills — la app y el CLI de **OpenAI Codex** primero, Cursor y OpenCode después — sin bifurcarlo. El diseño es **núcleo portable + ejecutores locales al runtime**: el manual operativo canónico vive en `AGENTS.md` (raíz, agnóstico; `CLAUDE.md` queda como capa fina específica de Claude Code), los 26 skills se descubren vía `.agents/skills/`, y los agentes del equipo tienen proyecciones generadas para Codex (`.codex/agents/*.toml`, con tiers neutrales MECH/STANDARD/JUDGE mapeados por runtime). El estándar operativo es `agent-portability.md` (PORT-1…6). **El motor Dynamic Workflow y su supervisor siguen siendo exclusivos de Claude Code.** Desde el **2026-09-02 (DR-120)** la capacidad de escritura de Codex está **congelada**: *cualquier* runtime que no sea Claude Code —Codex, Cursor, OpenCode— es **solo lectura/review** sobre el estado de build. El perfil `attended_foreground` que existió entre el 2026-07-15 y el 2026-09-02 queda **retirado** (no llegó a ejecutar ni un build real), y las certificaciones R10/R11 quedan **suspendidas, no fallidas**. Disparador para reabrir, literal: *"Codex ships wake-capable local scheduling"*. Los ficheros permiten una continuación posterior, no takeover vivo: si algún día se reabre, el cambio de runtime ocurriría únicamente después de un safe point limpio, parada completa y liberación de ownership. Cada runtime usa sólo sus propios agentes/modelos. Los gates humanos, el idioma (con el propietario siempre en español) y la disciplina documental no degradan nunca.
