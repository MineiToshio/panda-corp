...
| **Tribunal del Juez** | Órdenes esperando o pasando revisión; la cola de gates. Desde 9.116.0 hasta 2 FRDs pasan por tribunal a la vez por defecto (cada uno en su propio worktree; se apaga con `--no-parallel-gates`), pero el veredicto sigue aterrizando a main de uno en uno. El tribunal **abre** con el evento `gate` y **cierra** con el `GateVerdict`: cuando el juez dicta veredicto, la sesión de ese FRD termina | Frontmatter `IN_REVIEW` + eventos `gate`/`GateVerdict` del motor |
