...
- **Responder algo que te preguntó** → `/pandacorp:decide`. Te muestra la decisión con su recomendación, la registra en `decisions.md` y desbloquea el frente. Si hay varias pendientes, cada tarjeta del Resumen trae su propio comando `/pandacorp:decide <id>` que apunta a ESA decisión (sin el id, te las muestra todas una por una). Si una decisión tiene **7 días o más**, el skill la marca y te pregunta si sigue vigente ANTES de pedirte que respondas — si ya no aplica, la registra como **OBSOLETA** (no RESUELTA: no hace falta inventar una respuesta a algo que el codebase ya superó). Toda resolución — la respondas tú o la resuelva un agente solo durante una build — se marca **en el bloque original** de `decisions.md`, nunca solo en una entrada nueva arriba: si no, la tarjeta se queda "pendiente" para siempre aunque ya esté todo decidido.
