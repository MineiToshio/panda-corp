...
describe("FRD-07 gate (opus) — reverse cross-navigation: agent detail lists using-skills", () => {
  ...
  it("pressing Back after a reverse cross-nav closes the skill detail (gate reject #4: the Back button was dead because the detail was prop-controlled)", async () => {
    const target = findAgentWithUsingSkills();
    if (!target) return;

    await renderRealPage();

    // agent → skill (reverse cross-nav) opens the skill detail via the controlled prop
    fireEvent.click(screen.getByTestId("config-tab-agents"));
    const allCards = screen.queryAllByTestId("agent-card");
    const agentCard = allCards.find((el) => el.getAttribute("data-agent-id") === target.agentId);
    if (!agentCard) return;
    ...
    expect(
      ...
      "skill detail must be open after the reverse cross-nav",
    ).not.toBeNull();
    ...
    expect(
      screen.queryByTestId("skill-detail"),
      "Back must close the cross-nav-opened skill detail and return to the skills list (no dead button)",
    ).toBeNull();
  });
});
