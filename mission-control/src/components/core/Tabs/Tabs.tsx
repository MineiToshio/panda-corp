...
export interface TabsProps {
  ...
  testIdPrefix?: string;
  /**
   * Optional prefix for each tab button's HTML `id` attribute
   * (e.g. `"config-tab-id-"` → `id="config-tab-id-skills"`).
   * Required when a paired `role="tabpanel"` uses `aria-labelledby` to
   * reference the tab button's id. When omitted, no `id` is set on the button.
   */
  tabIdPrefix?: string;
}
