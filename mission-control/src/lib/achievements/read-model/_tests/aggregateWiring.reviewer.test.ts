...
/** Every file path under `root`, recursively, relative to `root` (empty when `root` is absent). */
...
describe("AC-23-003.1 — the aggregate index is reachable from the running system, not just unit-tested", () => {
  ...
  it("some real MC read path consumes readStatsAggregate (the O(1) index is actually read)", () => {
    ...
    const isSourceFile = (rel: string): boolean =>
      /\.(ts|tsx)$/.test(rel) && !rel.split(path.sep).includes("_tests");
    const consumers = roots
      ...
      .filter((file) => readIfExists(file).includes("readStatsAggregate"));
