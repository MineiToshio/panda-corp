...
- **Read-only over the factory.** The app only reads files and never calls Claude; the only writes are a small, bounded, human-triggered set on one idea card's frontmatter — discard/restore `status:` (FRD-02, ADR-0002) and the visual `favorite:` flag (FRD-02 REQ-02-012, ADR-0003).
