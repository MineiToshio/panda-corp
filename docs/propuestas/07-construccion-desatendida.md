...
3. **Driver orientado a meta**, no a intervalo: "construye hasta que la cola esté vacía y todos los FRD tengan e2e verde". Encaja con `/loop` *self-paced* o `/goal`.
...
- **`plugin/skills/implement/SKILL.md`**: documentar arranque en auto mode; explicitar commit-solo-en-borde-de-WO-en-verde; añadir provisioning de worktree (.env + deps); convertir "3 errores = parar" en **freeze-on-red** (no commitear lo roto, dejar HEAD en `last_green_sha`, marcar WO `BLOCKED`, **PushNotification a Sergio**, seguir con WOs independientes); `TaskStop` de dev servers al cerrar cada FRD.
- **`/loop` — SÍ y NO:** SÍ como *self-paced*/`/goal` ("corre hasta vaciar la cola y para solo"). NO a intervalo fijo (los scheduled tasks expiran a 7 días, son session-scoped, no hacen catch-up, y meten delays de 1min–1h). Para trabajo continuo: loop de Agent Teams + Monitor.
