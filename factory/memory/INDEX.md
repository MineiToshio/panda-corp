...
- LESSON-0027 · use when about to assert a project/feature's current state by citing a prior audit/proposal finding, OR before executing a fix/change plan derived from one, rather than checking the current code · an audit finding (or a fix plan built on it) is a snapshot, not a durable fact; re-verify against the live artifact — for a multi-item plan, red-team each item against the live file before executing
