...
source: "project personal-page-v2 — overlay upgrade 8.47.0→8.51.0 overwrote biome.json (project had schema 2.5.1 + preset:recommended for biome 2.5.1) with the template's stale schema 2.5.0 + deprecated recommended:true; biome 2.5.1 errors on that under --error-on-warnings, so the pass-2 baseline self-heal went RED and blocked the build (builtFrds:[], blockedReasons:{baseline:error}). CORROBORATED WORKING 2026-07-07 — a later /upgrade of the same project (overlay 8.51.0 to 8.69.0) hit a related case: `detect-gate-config-newer.sh` (the DR-076/BL-0003 backstop this lesson led to) returned `unknown` for `knip.json` (its schema is project-shaped — a custom `entry`/`ignores` list, not a simple comparable semver like biome's `$schema`), and the project's knip.json was correctly PRESERVED rather than blindly overwritten, per DR-094 — confirming the backstop's judgment call (preserve on `unknown`, don't force-template) holds in practice, not just in the original incident. FURTHER FACET, 2026-09-06/07 (same project, `.pandacorp/run/lessons.md`, harvested 2026-09-08): during a DR-059 conformance sync, an agent noted that `git log` on the canonical gate file BEFORE overwriting it is itself a useful discriminator, generalizing beyond schema-comparable configs (biome's `$schema`, which `detect-gate-config-newer.sh` can parse) to ANY canonical file including ones with no comparable version field (knip.json, playwright.config.ts) — a recent, deliberate commit touching the file is a strong signal to merge/skip rather than clobber, complementing (not replacing) the `unknown`-schema preserve-by-default backstop. RELATED FACET, 2026-09-08 (same project, `.pandacorp/run/lessons.md`): a different symptom from the SAME general class (what the record says vs what the tree currently holds can silently diverge) — a gate REVIEWER always runs against `HEAD` (its gate-worktree is checked out at the commit, confirmed via `git worktree list`), so an edit a builder left ONLY in the main checkout's working tree (uncommitted) is invisible to the reviewer's tree but can still be described as done in the Status Note/decision-log, producing a phantom regression finding for something already fixed in the working tree but not yet committed. Diagnostic: `git show HEAD:<file> | grep <term>` vs a plain `grep <term> <file>` on the working tree — if they differ, the problem is the commit boundary, not the code. OPPOSITE-DIRECTION FACET, 2026-09-11 (same project, `.pandacorp/run/lessons.md`, WO-08-006, second occurrence in this project): the same HEAD-vs-working-tree divergence can also produce a false NEGATIVE, not just a false positive — `privacyGate.ts` was committed importing a NEW file (`piiRules.ts`) that a serialized-writer build batch never added to the commit; `HEAD` was structurally broken (a committed import with no committed target) while the working tree (which still had the uncommitted new file on disk) passed every test, because vitest reads the working tree, not the git index. Cheap check to add at the close of each WO in a serialized-writer build: `git ls-files --others --exclude-standard <dir>` (lists untracked files under a directory) before declaring a WO's gate green, or run the gate against a `git archive` of HEAD instead of the raw working tree."
...
**Apply next time — new facet (2026-09-11):** a serialized-writer build (one committer, sequential WOs) can
leave `HEAD` broken by an UNCOMMITTED new file that a committed file already imports, while the working
tree (which still has that file on disk) passes every test — the false-negative twin of the phantom-finding
false positive above. Both share the same root cause: what git's committed record says and what the
working tree currently holds can silently diverge in a multi-writer build. Close each WO with a cheap
untracked-file check (`git ls-files --others --exclude-standard <dir>`) or gate against a `git archive` of
`HEAD`, not just the raw working tree.
...
syncing — never assume the sync is safe. For an agent doing this check MANUALLY (not via
`detect-gate-config-newer.sh`), `git log` on the canonical file before overwriting is a cheap, general
discriminator that works even when the file has no comparable version field: a recent, deliberate commit
is a strong signal to merge/skip rather than clobber.
