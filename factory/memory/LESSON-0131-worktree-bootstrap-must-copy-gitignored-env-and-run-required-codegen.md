...
**Fifth occurrence (2026-09-09):** the same two gaps reproduced yet again in a fresh worktree. This time
the `next build`-only codegen fix landed for good in-session; `.env.local` was still missing, and it is
STILL the case (five occurrences in) that no `.pandacorp/worktree-setup.sh` hook exists on this project —
each occurrence keeps fixing the symptom in front of it (or one half of it) rather than creating the hook
once. If you land on this project again with either symptom, creating the hook is the fix, not another
investigation.
