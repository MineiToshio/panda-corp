...
created: 2026-07-07
status: candidate
promotion: proposed   # 2026-08-11 (librarian review, refined from 2026-07-28) — target factory/standards/infra.md (hooks section): now corroborated NINE times across 3 distinct projects (mission-control x2, personal-page-v2 x2, panda-corp x5) and 5 distinct trigger surfaces (commit-message prose, a Co-Authored-By trailer, a genuine single-line-stamp redirect, a grep search pattern/OR-alternation containing an arrow token, and a plain HTML tag/comment-close token inside an ordinary grep pattern); document the redirect-guard's known false-positive surface + the rephrase/`-F`/`-nF --`/Read-tool workarounds as a standing convention until BL-0047 ships the quote-aware fix
confidence: medium
times_applied: 0
applied_in: []
