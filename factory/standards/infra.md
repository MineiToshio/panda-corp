...
**Restore on a new machine (bring the whole factory back).** The full state lives in a few disjoint sources; restore in order:
1. **Toolchain** — install the runtime (fnm+node, pnpm), Claude Code and the `pandacorp` plugin.
2. **The factory repo** (code/framework) — `git clone` `panda-corp` into `Proyectos/`. Personal files are gitignored, so `factory/ideas`, `profile.md` etc. come back EMPTY here.
3. **The vault overlay** (personal data) — clone the private repo as the bare overlay and check it out over the factory tree, which refills the personal files in place:
   `git clone --bare https://github.com/<owner>/pandacorp-vault.git <Proyectos>/pandacorp-vault/personal.git` then
   `git --git-dir=<Proyectos>/pandacorp-vault/personal.git --work-tree=<Proyectos>/panda-corp checkout`.
4. **Each sibling product project** — `git clone` its own repo into `Proyectos/`. Their `.pandacorp/` owner-state (inbox/comms) is recovered from the **offsite backup** (restore `pandacorp-vault/backups/` from the cloud and copy each project's `.pandacorp/` back).
5. **Deps + deploy + secrets** — `pnpm install`; re-run the local deployment (`deploy-local.sh`, or reinstall the launchd plist); restore the SOPS **age key** into `~/.config/pandacorp/` from your password manager (it is deliberately NOT in any repo or the vault).
...
- **Gitignored-data gotcha (`PANDACORP_FACTORY_ROOT`).** The deployment is a git **worktree**, which carries only TRACKED files — the owner's **gitignored** state (for MC: `factory/profile.md`, `ideas/*.md`, portfolio) does NOT exist in it. An app that reads that live state (MC is a live dashboard over the factory filesystem) MUST be pointed at the **main checkout** via `PANDACORP_FACTORY_ROOT`, or it reads an empty `factory/` and trips the **onboarding gate** ("no hay perfil"). `serve.sh` MUST `export PANDACORP_FACTORY_ROOT=<main checkout>` (derive it from the shared `.git`: `dirname "$(git -C "$DEPLOY_DIR" rev-parse --git-common-dir)"`). Same class as the PATH gotcha — a launchd-clean, worktree-isolated deploy needs its live-data source wired explicitly.
...
- **The machinery (`.pandacorp/run/serve.sh` + `deploy-local.sh`) is gitignored, regenerable runtime.** It was lost once (the whole `.pandacorp/run/` got swept — the BL-0035 protected-path class), which is what took the deploy down. Since v9.72.0, `backup-pandacorp-state.sh` backs up `run/*.sh` + `run/lessons.md` for every project's `.pandacorp/` layer, so this instance-specific machinery is no longer unrecoverable — but it is still precious runtime state: reconstruct from this reference if needed, never delete the dir.
...
- **Removal is blocked mechanically, not just by convention (BL-0151).** `deploy-local.sh` re-asserts `git worktree lock` on the deploy worktree at the top of every redeploy (idempotent — a lock on an already-locked worktree is a harmless no-op error, swallowed). Independently, `block-dangerous.sh` (the PreToolUse gate) refuses `git worktree remove`/`rm -rf` whenever the target is under `local-deployments/**` OR is reported `locked` by `git worktree list --porcelain` — the path-prefix rule alone would have caught the 2026-09-22 incident (the worktree was not yet locked when it was removed), and the lock-state rule generalizes the same protection to any other worktree an operator pins by hand, without hardcoding a second project's path.
