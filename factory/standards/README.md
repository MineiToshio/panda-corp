...
| **Factory operation / Portability** | [agent-portability.md](agent-portability.md) (one portable core with strict runtime-local executors: capability matrix, model tiers, tool translation, R0 read/review boundary and safe-point-only cold continuation — DR-113) · [memory-harvesting.md](memory-harvesting.md) (evidence or nothing: the anchor gate on `factory/memory/` — DR-047, promoted from LESSON-0001) |
