...
- **Visual-fidelity gate (DR-056) — does the build MATCH the mock, not just render clean.** The
  ...
  the `reviewer`.
- **Responsive gate (DR-074) — does the build actually WORK at a mobile width.** Runs inside
  `verify.sh`, fail-closed, but only at the **full** unscoped run and close-out (DR-106) — the
  per-FRD `--since` gate does not run it (only `smoke` + `shell` do, for wall-clock reasons; see
  DR-106 below). When the project's `target_platforms` (`.pandacorp/status.yaml`) includes mobile, it asserts at the mobile
...
- **Nothing is left in a bare whole-project run** — every FRD is `VERIFIED`, then the **final hardening step** runs (DR-085: security audit + quality close-out + telemetry/metrics verification — the audit that used to live in `/pandacorp:release` is now construction's last step) → `phase: release`. Reaching `release` means the build is hardened and ready to be **launched** (deployed internal or external) by `/pandacorp:release`; there is no `operation` phase after it. A targeted FRD/change run owns only its requested scope: after its complete per-FRD gates it quiesces with `phase: implementation`, even when that scope happened to be the last globally pending work. Only a later bare run may audit/fix the whole project and advance release. Codex's `attended_foreground` candidate remains unavailable while policy is `FALLBACK`; after its installed canary and explicit promotion it is narrower still: exactly one FRD or ready change, foreground, cumulative duration <=7200 seconds and zero automatic restart, never this bare close-out.
