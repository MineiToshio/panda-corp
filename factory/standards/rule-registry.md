...
Maintained by hand; `check-standards.sh` validates the template shape and that every standard file is represented here. Update the row in the same change that adds/changes/wires a rule (`/pandacorp:learn` step 6 runs the checker and reports its exit code; the weekly `pandacorp-consistency-sweep` routine runs it too — BL-0055). Last full audit: 2026-07-02 (docs/proposals/24).
...
| CONV-13 | Claims and diagnoses: evidence before assertion — no fact asserted to the owner, and no internal diagnosis/fix plan/"done" built, without an in-session tool observation; recorded state (flags, cached counts, subagent reports, prior audits, docs, recognized patterns, self-reviews) is a stand-in to re-verify, never repeated; the unverified is labeled unverified | MUST | conventions.md | agent self-check before asserting or diagnosing + owner spot-check (chat face of constitution §22/§24; build face wired via Stop gate + DR-068) | manual |
| CONV-14 | Verified traps: no lexicographic ISO-8601 comparison across producers (`Date.parse` or one pinned format); `matter(content, { excerpt: false })` never bare (gray-matter@4 content-keyed cache) + trailing-newline normalization on round-trip; `git add -u`/`-f` for a tracked path under a later-gitignored dir, never a weakened `.gitignore` | SHOULD | conventions.md | greppable `matter(` single-arg check (lint candidate); the other two review-only at symptom time | manual |
| CONV-15 | User-facing copy voice: no em dash (`—`), no en dash as sentence separator, in product copy (UI/i18n, MDX, blog, metadata, generated drafts) — the AI-prose tell; factory prose, code and owner chat excluded | SHOULD | conventions.md | `grep` check on an automated content pipeline's output where one exists; reviewer + owner spot-check otherwise | manual |
...
| QUAL-14 | Owner-attended live attempts: red-team the FULL prior-failure matrix + prove the rehearsal exercises the real boundary (≥1 non-mocked element, negative corpus RED) before consuming another | MUST | quality.md | NAMED pre-request step in the requesting flow + owner spot-check | manual |
| DEBUG-1 | Reproduce/obtain the artifact first; establish the timeline (last-known-good, what changed, parallel sessions) | MUST | debugging.md | bug-skill repro capture + BL-item Root cause section | manual |
| DEBUG-2 | Full data path read; suspects enumerated and discarded one-by-one with direct evidence — ALWAYS incl. parallel writers and the harness/environment itself | MUST | debugging.md | BL-item Root cause section + owner spot-check | manual |
| DEBUG-3 | Confirmed cause predicts the symptom AND explains the non-failures; sibling audit before closing; fix exercised on the real flow + regression test (DR-015) | MUST | debugging.md | reviewer FRD gate (regression test) + BL-item Done-when | manual |
| DEBUG-4 | An unexplained incident closes with attribution guards (recurrence attributable + non-destructive) and a ruled-out record, never as "mystery" | MUST | debugging.md | BL-item Done-when (BL-0035 pattern) + owner spot-check | manual |
...
| PORT-5 | EVERY non-Claude runtime is read/review-only on project build state — no build writes anywhere (DR-120 freeze, 2026-09-02); the former Codex attended-foreground profile is withdrawn and R10/R11 are suspended | MUST | agent-portability.md | capability-policy gate (`check-skill-capabilities.mjs` asserts `implement.codex: FALLBACK` + `codex_product_executor: FROZEN`); enforcement-side wiring is BL-0030 | wired |
...
| DOCC-1 | Fresh-set internal coherence: a newly generated set (spec = PRD + v1 FRDs; architecture = architecture.md + blueprints + work orders) comes out with no hard internal contradiction; generator never grades its own set | MUST | document-consistency.md | fresh verifier agent in the `spec`/`architecture` phase gates — a hard contradiction keeps the set `DRAFT` (evidence-stamped like `readiness_gate`, DR-100) | manual |
| DOCC-2 | Supersession-completeness: an edit that REPLACES a rule/claim declares the old claim and propagates it everywhere — no surviving old statement in `factory/`, `plugin/` or `docs/` (dated decision-log history excluded, DR-093). Blocks incomplete propagation, never the change | MUST | document-consistency.md | supersession-completeness check named in `learn` step 5b and in `change`/`iterate` before the change is done | manual |
| DOCC-3 | Bounded detection: candidate set derived from the declared supersession + the AGENTS.md canonical-doc table + grep, then read semantically; never a whole-corpus read nor a raw string match | SHOULD | document-consistency.md | the same `learn`/`change`/`iterate` step (delegated STANDARD-tier grep sweep with a COMPLETE-list contract; verdict stays in the main thread) | manual |
| DOCC-4 | Drift between changes is swept periodically; the sweep reports and files `BL-*`, never edits and never blocks | SHOULD | document-consistency.md | `pandacorp-consistency-sweep` routine (`plugin/docs/routines.md`, weekly cron) — advisory by design | manual |
| DOCC-5 | Promise without mechanism: a new/edited claim that something triggers, enforces or regenerates names its installed call site as a `path:line`, ships the wiring in the same change, or cites a tracked `BL-*` inline | MUST | document-consistency.md | author self-check visible in the same diff + `reviewer`/consistency sweep on un-pointed trigger claims; the standards-catalog slice is wired via `check-standards.sh`'s aspirational-MUST burn-down | manual |
| SSOT-1 | One writer per fact: every fact (count, status, level, rollup, config value) has ONE authoritative home; every other surface derives. A second independent derivation is a defect | MUST | single-source-of-truth.md | reviewer FRD gate (quality lens, DR-115/DR-092) | manual |
| SSOT-2 | Derive-on-read is the default, through ONE shared resolver; where a resolver for the concept exists, calling it is the only permitted access | MUST | single-source-of-truth.md | reviewer FRD gate (rejects a private re-derivation) | manual |
| SSOT-3 | Honest cache (the exception): a stored derived value needs one NAMED writer, re-derivation from the atomic source at defined safe points (never scattered increments), a replica note at the field/file, and no UI read while a live resolver exists | MUST | single-source-of-truth.md | reviewer FRD gate + the engine's re-derivation at safe points (DR-050 §1) | manual |
| SSOT-4 | Enforcement by construction: retiring a stale stored copy also deletes the field from reader types/mappings in the same change | MUST | single-source-of-truth.md | reviewer (the change is rejected without it); once removed, `tsc` holds the regression | manual |
| SSOT-5 | `status.yaml` keys absent from the canonical template schema are flagged as written-but-never-declared drift | SHOULD | single-source-of-truth.md | doc-lint schema-drift check (advisory) | manual |
...
| INFRA-5 | New gitignored state answers both durability questions in the change that introduces it: is its loss a real incident (→ into `backup-pandacorp-state.sh`) and does a write/regeneration trigger actually fire; worktree addendum — provisional gitignored notes are written to the MAIN checkout's copy | MUST | infra.md | NAMED design-time question in the introducing change; `backup-pandacorp-state.sh`'s path list is the receipt | manual |
| INFRA-6 | A safety gate that matches surface TEXT (hook or model classifier) is never weakened to get unblocked — change how the intent is expressed (file over inline string, rephrase, `git commit -F`, `grep -nF --`, Read over grep); a gate false-positiving across unrelated call sites is filed as a `BL-*` | MUST | infra.md | agent self-check at the moment of a block + the `BL-*` filing rule (BL-0047, BL-0120 open) | manual |
...
| BUILD-2 | A gate's verdict is the GATE's own exit code, never `$?` after a bare pipe (`set -o pipefail` / `${PIPESTATUS[0]}` / run un-piped) | MUST | build-orchestration.md | agent/script self-check when reading a gate result; the false-green direction, review-only today (pipefail-in-wrapper lint is the wiring candidate) | manual |
| BUILD-3 | A broad e2e/`webServer` failure is triaged for port collision and an orphaned dev-server lock (reserved `PORT` from `factory/ports.yaml`, `lsof`, fresh boot over reuse) before being called a regression | MUST | build-orchestration.md | NAMED triage step before a red e2e is diagnosed; BL-0037/BL-0049 track the code-side fix | manual |
...
| PROMPT-8 | Runtime-synthesized dispatch prompts are self-contained on-camera briefs (role+boundary, owned files, embedded commands, output contract, verbatim DR-108 context pack) — never pointer specs | SHOULD | prompting-conventions.md | edit-time self-check on engine/skill prompt-construction changes + reviewer of engine edits | manual |
...
## Counts (derived by `check-standards.sh`; no hardcoded number — BL-0091)

The total count and enforcement-status distribution are derived live by `factory/standards/check-standards.sh`'s `awk` recount over this table (header/separator/structural rows excluded) — run `bash factory/standards/check-standards.sh` to see the current numbers rather than trusting prose here (the previous hardcoded header (138) had already drifted from the live table (137) before BL-0091; a naive `^\|` grep over-counts at 143 by including non-rule rows). Snapshot 2026-09-03 (`bash factory/standards/check-standards.sh` output): 157 rules → 33 wired · 123 manual · 1 aspirational (SHOULD) · 0 aspirational MUST. The 2026-09-03 memory-promotion sitting (proposal 33 §12.4) added 7 rows, all `manual`, each promoted from a corroborated `factory/memory/` lesson: **CONV-14** (verified traps — LESSON-0005/0009/0103), **CONV-15** (user-facing copy voice — LESSON-0123), **DOCC-5** (promise without mechanism — LESSON-0113), **INFRA-5** (new gitignored state needs a backstop and a firing write trigger — LESSON-0090), **INFRA-6** (safety gates match text: change the surface, never the gate — LESSON-0105/0109), **BUILD-2** (a gate's verdict is its own exit code — LESSON-0078) and **BUILD-3** (triage a broad e2e failure before calling it a regression — LESSON-0040); **CONV-13** was widened in the same sitting to cover internal diagnoses and fix plans (LESSON-0069). DOCC-1..4 (document consistency, DR-116) and SSOT-1..5 (single source of truth, DR-115) added 2026-09-02 (BL-0055) — both standards had shipped without registry rows, which held `check-standards.sh` RED since 2026-07-09; all nine are `manual` with NAMED steps (phase-gate fresh verifier, `learn` step 5b, the `pandacorp-consistency-sweep` routine, the reviewer FRD gate), honestly registered — none is `wired`. CONV-13 (owner-facing claims: evidence before assertion — owner-stated 2026-06-20, previously only personal memory), QUAL-14 (owner-attended live-attempt economy — owner-stated 2026-07-12, R10 series) and DEBUG-1..4 (debugging/incident SOP, new `debugging.md` — BL-0035 precedent generalized) added 2026-07-15, all `manual` with NAMED steps. PROMPT-1..7 (prompting conventions for the agent/skill prompt surface, DR-114, Fable sprint WS3) added 2026-07-04 — PROMPT-5 wired via `check-derived-drift.sh` + `claude plugin validate`; the rest `manual` with the NAMED edit ritual (PROMPT-6 fresh-context verifier pass). PROMPT-8 (runtime-synthesized dispatch prompts; promoted from LESSON-0147, ringer external-repo mining) added 2026-07-15 — manual. PORT-1..6 (multi-runtime portability, DR-113) added 2026-07-04 — all `manual` with NAMED steps (AGENTS.md-injected tables, Codex verification rounds, file-based gates); the derived-copy drift check is now **wired** (`plugin/scripts/check-derived-drift.sh`, Stop hook in the factory: TOML regen diff + manifest version sync + `.agents/skills` symlink, fail-closed + self-tested — 2026-07-04, closes audit-20 P1-16's mechanical slice); the preflight-drift check is now **wired** too (`plugin/scripts/check-preflight-drift.sh`, Stop hook, BL-0091). DOC-3 (root README populated, DR-112) added 2026-07-03. CONV-12 (subagent model selection, DR-111) added 2026-07-02. The 2026-07-02 catalog expansion (error-handling, data-modeling, auth, resilience, background-jobs, dependency-lifecycle, feature-flags, ai-implementation + PAT-10/11) added 32 rows, honestly registered: mostly `manual` with NAMED steps (reviewer lenses, blueprint readiness gate, security-auditor/hardening checklists, human gates) — each is a wiring candidate where a lint/gate can hold it. Health metric: wired share should only go up; the aspirational-MUST list stays at zero — `check-standards.sh` warns if one reappears.
