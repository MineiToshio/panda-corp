...
| PORT-5 | Non-Claude runtimes remain read/review-only on build state under the R7 NO-GO verdict; runtime switches are cold, safe-point-only, one runtime at a time | MUST | agent-portability.md | supersession grep + R7/R8/R11 reevaluation set; offline executor tests alone never promote | manual |
